...
const AnimatedCounter = ({ value, duration = 2, decimals = 0, prefix = '', suffix = '' }) => {
  ...
  return <span>{prefix}{isNumeric ? display.toLocaleString() : value}{suffix}</span>;
};
        ...
        <div className="max-w-6xl mx-auto relative">
          ...
          <div className="flex flex-col lg:flex-row gap-8">
            ...
            <div className="flex-1 min-w-0 space-y-6">
              ...
              >
                {[
                  { label: 'LC Solved', numeric: lcTotal, display: lcTotal, color: '#FFC22D', icon: 'M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z' },
                  { label: 'CF Rating', numeric: cfRating || 0, display: cfRating || 'N/A', color: '#60a5fa', icon: 'M11.049 2.927c.3-.921 1.603-.921 1.902 0l1.519 4.674a1 1 0 00.95.69h4.915c.969 0 1.371 1.24.588 1.81l-3.976 2.888a1 1 0 00-.363 1.118l1.518 4.674c.3.922-.755 1.688-1.538 1.118l-3.976-2.888a1 1 0 00-1.176 0l-3.976 2.888c-.783.57-1.838-.197-1.538-1.118l1.518-4.674a1 1 0 00-.363-1.118l-3.976-2.888c-.784-.57-.38-1.81.588-1.81h4.914a1 1 0 00.951-.69l1.519-4.674z' },
                  { label: 'LC Ranking', numeric: lcData?.ranking || 0, display: lcData?.ranking ? `#${lcData.ranking.toLocaleString()}` : 'N/A', color: '#4ade80', icon: 'M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z', prefix: '#' },
                ].map((stat) => (
                  <div
                    key={stat.label}
                  ...
                  >
                    <div className="relative z-10">
                      ...
                      <p className="text-2xl sm:text-3xl font-bold tracking-tight" style={{ color: stat.color }}>
                        {typeof stat.display === 'string' && stat.display === 'N/A' ? (
                          'N/A'
                        ) : stat.prefix ? (
                          <>{stat.prefix}<AnimatedCounter value={stat.numeric} duration={1.5} /></>
                        ) : (
                          <AnimatedCounter value={stat.numeric} duration={1.5} />
                        )}
                      </p>
                    </div>
                  </div>
